...
def process_data(dataset,freq='D'):                       #归一化   dataset=log(A*1000)
...
def predict(dataset,days=2):
    ...
    if success == True :
        ...
        #dataset = result_arma.predict(len(dataset)-days, len(dataset), dynamic=True)    #已改
...
def forecast(dataset,delta=2,freq='D'):
