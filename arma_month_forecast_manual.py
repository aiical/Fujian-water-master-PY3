...
# 预测日期
FORECAST_START_DATETIME=FIRST_DAY
FORECAST_END_DATETIME=dateshift(FIRST_DAY,RUN_LEN-1)
...
def arma_forecast(dataset,column):
    datas = arma.forecast(dataset,delta=6,freq='MS')
    if datas is None:
        return datas
    values = datas.values
    index = datas.index
...
if __name__ == '__main__':
    ...
    for site in sites:
		...
		if arma_datas.values[0] is not None:
                    pass
                    save_month_forecast_data(FORECAST_START_DATETIME,site['id'],arma_datas.values,factor['code'],model="ARMA")
            else:
